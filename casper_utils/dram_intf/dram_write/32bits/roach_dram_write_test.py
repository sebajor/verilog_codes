...
@cocotb.test()
async def roach_dram_write_test(dut, iters=1024, din_width=32, multiplex_in=1,
        dout_width=288, multiplex_out=9, burst_write=3, sleep_write=100,
        continous=0):
    ...
    if(continous):
        ...
        while(counter<len(data)):
            ...
            for i in range(burst_write):
                if(counter<len(data)):
                    dut.din.value = int(data[counter])
                    await ClockCycles(dut.clk,1)
                    counter +=1
                else:
                    break
