...
@cocotb.test()
async def dram_tge_test(dut, burst_size=4,tge_pkt_size=1024,wait_pkt=32):
    ...
    dut.rst <= 1;   dut.en <=0
    dut.tge_pkt <= tge_pkt_size;
    dut.tge_wait <= wait_pkt;
    dut.tge_total_pkts <=  int(tge_total_pkts)
    ...
    dut.en <= 1
    await tge_write(dut)
    await ClockCycles(dut.clk, 2*1024)
...
async def dram_read(dut):
    ...
    while(1):
        ...
        if(req==1):
            ...
            if(flag ==0):
                flag =1
                delay_rsp = np.random.randint(32)
        if((delay_rsp==0) & flag):
            if(dram_reqs.empty()):
                flag =0
                dut.dram_valid <=0
