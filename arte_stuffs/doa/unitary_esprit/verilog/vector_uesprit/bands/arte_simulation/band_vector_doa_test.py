...
@cocotb.test()
async def point_doa_no_la(dut, iters=10, acc_len=32, vec_len=2048,bands=8,
    ...
    if(collect_phases): 
        ...
        gold_dout = np.zeros([len(freq), len(gold_phases[str(int(freq[0]))])])
        rtl_dout = np.zeros([len(freq), len(gold_phases[str(int(freq[0]))])])
        for i in range(len(freq)):
            ...
            rtl_dout[i,:] = rtl_phases[str(int(freq[i]))]
            gold_dout[i,:] = gold_phases[str(int(freq[i]))]
