...
async def moving_average_test(dut, iters=128, win_len=16, din_width=32, din_point=31):
    ...
    dat = np.random.random(40)-0.5
    #dat = np.arange(40)/100
    #dat[::2] = -dat[::2]
    #test = await rnd_test(dut,dat, win_len, din_width, din_int, thresh=0.05)
    #dat = -np.ones(30)*0.5
    test3 = await burst_test(dut, dat, win_len, din_width, din_int, burst_len=6, thresh=0.5)


async def burst_test(dut, dat, win_len, din_width, din_int, burst_len, thresh):
    ...
    for i in range(len(out_values)):
        #print(i)
        #print("gold: %0.5f"%(2*gold_values[i]))
        #print("out: %0.5f \n"%(2*out_values[i]))
