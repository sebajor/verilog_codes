...
async def signed_acc_test(dut, iters=128, din_width=16, dout_width=32, acc_len=10,
    ...
    #gen data
    data = np.random.randint(0, 2**8, size=[acc_len, iters])*(-1)**(np.random.randint(0,2,size=[acc_len, iters]))
    gold = np.sum(data, axis=0)
    
    dat = two_comp_pack(data.flatten(), din_width,0).reshape(data.shape)

    cocotb.fork(read_data(dut, gold, dout_width, 0))
    await write_data(dut, dat, cont, burst_len)
