...
@cocotb.test()
async def correlation_matrix_test(dut, acc_len=5, vec_len=64, din_width=16, din_pt=14,dout_width=32, dout_pt=16):
    ...
    for j in range(len(r11)):
        print(j)
        print("gold r11  : %.4f \t out r11  :%.4f" %(gold_r11[j], r11[j]))
        print("gold r22  : %.4f \t out r22  :%.4f" %(gold_r22[j], r22[j]))
