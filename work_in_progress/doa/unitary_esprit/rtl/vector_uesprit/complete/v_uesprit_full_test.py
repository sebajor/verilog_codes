...
@cocotb.test()
async def v_uesprit_la_test(dut, acc_len=16, vec_len=64, din_width=16, 
        din_pt=15, dout_width=16, dout_pt=13, xspace=0.01):
    freqs = np.ones(20)*11
    phases =np.ones(20)*70
    ...
    l1, l2, e1,e2,e_frac,phase = await write_data(dut,spec0,spec1,din_width,din_pt,dout_width,dout_pt) 
    l1, l2, e1,e2,e_frac,phase = await write_data(dut,spec0,spec1,din_width,din_pt,dout_width,dout_pt) 
    ...
    gold_phase = np.arctan2(gold_e1, gold_frac)/np.pi
    print("gold phase:%.4f \t rtl phase:%.4f"%(gold_phase[int(freqs[0])], phase[int(freqs[0])]))
    ...
    for i in range(len(phase)):
        print(i)
        print("gold phase:%.4f \t rtl phase:%.4f"%(gold_phase[i], phase[i]))
...
async def write_data(dut, dat1, dat2, din_width, din_pt, dout_width, dout_pt):
    ...
    for i in range(acc_len):
        ...
        for j in range(vec_len):
            ...
            val_phase = int(dut.phase_valid.value)
            if(val_phase):
                ang = int(dut.phase1.value)
                ang = two_comp_unpack(np.array([ang]), dout_width,1)
                phase.append(ang)
    out_vals = [lamb1, lamb2,eig1,eig2,eig_frac,phase]
    return out_vals
