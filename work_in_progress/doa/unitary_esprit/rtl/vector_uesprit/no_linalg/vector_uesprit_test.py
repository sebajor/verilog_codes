...
@cocotb.test()
async def vector_uesprit_test(dut, acc_len=5, vec_len=64, din_width=16, 
    ...
    for i in range(1,len(phases)):
        ...
        spec1 = 1.*spec1/(np.max([np.max(spec1.real), np.max(spec1.imag)]))
        r11,r22,r12_re,r12_im = await write_data(dut,spec0, spec1, din_width,din_pt,dout_width, dout_pt)
        r11 = r11[int(freqs[i])]#np.sum(r11)
        r12 = r12_re[int(freqs[i])]#np.sum(r12_re)
        r22 = r22[int(freqs[i])]#np.sum(r22)
    ...
    """
    spec0, spec1 = create_clean_data(freqs=[10], phases=[-40],acc_len=acc_len, vec_len=vec_len,xspace=xspace)
    spec0 = 1.*spec0/(np.max([np.max(spec0.real), np.max(spec0.imag)]))
    spec1 = 1.*spec1/(np.max([np.max(spec1.real), np.max(spec1.imag)]))
    r11,r22,r12_re,r12_im = await write_data(dut,spec0, spec1, din_width,din_pt,dout_width, dout_pt)
    r11,r22,r12_re,r12_im = await write_data(dut,spec0, spec1, din_width,din_pt,dout_width, dout_pt)
    ##test
    gold_r11,gold_r12,gold_r22 = uesprit.uesprit_matrix(np.array([spec0, spec1]))
    for i in range(len(r11)):
        print(i)
        print("r11= gold:%.4f\t hdl:%.4f"%(gold_r11[i], r11[i]))
        print("r22= gold:%.4f\t hdl:%.4f"%(gold_r22[i], r22[i]))
        print("r12_re= gold:%.4f\t hdl:%.4f"%(gold_r12[i].real, r12_re[i]))
        print("r12_im= gold:%.4f\t hdl:%.4f"%(gold_r12[i].imag, r12_im[i]))
        print("\n")
    #classic uesprit
    r11 = r11[10]#np.sum(r11)
    r12 = r12_re[10]#np.sum(r12_re)
    r22 = r22[10]#np.sum(r22)
    norm = np.max([np.max(r11), np.max(r12), np.max(r22)])
    r11 = r11/norm; r12 = r12/norm; r22 = r22/norm
    doa, mu, l1,l2 = uesprit.uesprit_la(r11,r12,r22,x_space=xspace, freq=10)
    print("doa: {}, mu: {} l1,l2: {},{} ".format(doa, mu,l1,l2))
    """
