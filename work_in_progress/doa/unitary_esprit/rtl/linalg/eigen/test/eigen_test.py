...
async def eigen_test(dut, din_width=16, din_pt=15, dout_width=16, dout_pt=13, iters=128, thresh=0.05):
    ...
    #r11_val  = np.loadtxt('r11')/2.**15
    #r12_val  = np.loadtxt('r12')/2.**15
    #r22_val  = np.loadtxt('r22')/2.**15

    #r11_val = np.ones(iters)*r11_val[1365]
    #r22_val = np.ones(iters)*r22_val[1365]
    #r12_val = np.ones(iters)*r12_val[1365]
