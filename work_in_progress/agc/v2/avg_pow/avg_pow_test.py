...
@cocotb.test()
async def avg_pow_test(dut, iters=128, din_width=8,din_pt=7,delay_line=32):
    clk = Clock(dut.clk, 10, units='ns')
    cocotb.fork(clk.start())
    dout_width = 2*din_width
    dout_pt = 2*din_pt
    np.random.seed(20)
    #test1 = await cte_test(dut,delay_line,iters, din_width,din_pt,dout_width,dout_pt, thresh=0.05)
    test2 = await sin_test(dut,delay_line=32,iters=128, din_width=8,din_pt=7,dout_width=16,dout_pt=14, thresh=0.5)


async def test(dut,dat,iters=128,delay_line=32,din_width=8,din_pt=7,dout_width=16,dout_pt=14, thresh=0.05):
    ...
    for i in range(len(dat)):
        ...
        gold_vals.append(np.sum(mov)/len(mov))
        if(valid):
...
async def cte_test(dut,delay_line=32,iters=128, din_width=8,din_pt=7,dout_width=16,dout_pt=14, thresh=0.5):
    din_int = din_width-din_pt
    dout_int = dout_width-dout_pt
    dat = np.ones(iters)-0.5
    vals = await test(dut,dat,iters,delay_line,din_width,din_pt,dout_width,dout_pt, thresh)

async def sin_test(dut,delay_line=32,iters=128, din_width=8,din_pt=7,dout_width=16,dout_pt=14, thresh=0.5):
    din_int = din_width-din_pt
    dout_int = dout_width-dout_pt
    f = 30
    t = np.arange(iters)
    dat = 0.5*np.sin(2*np.pi*t*f/iters)
    vals = await test(dut,dat,iters,delay_line,din_width,din_pt,dout_width,dout_pt, thresh)
