...
async def msdft_test(dut, iters=180, dft_len=128, k=55, din_width=8,din_pt=7,
    ...
    #dat_im = np.ones(iters)*0.25
    #n = np.arange(iters)
    #dat_re = 0.6*np.sin(2*np.pi*k*n/dft_len)
    dat_im = np.zeros(iters)
    test1 = await msdft_rw(dut, dat_re, dat_im, dft_len,k,din_width,din_pt,dout_width, dout_pt)


async def msdft_rw(dut, dat_re, dat_im, dft_len=128, k=55, din_width=8,din_pt=7,
        dout_width=32,dout_pt=16):
    din_int = din_width-din_pt
    dout_int = dout_width-dout_pt
    dut.rst <=0;
    ...
    dut.din_re <=0; 
    dut.din_im<=0
    data_re = two_comp_pack(dat_re, din_width, din_int)
    data_im = two_comp_pack(dat_im, din_width, din_int)
