...
@cocotb.test()
async def moving_average_test(dut, iters=2048, win_len=128, din_width=25, din_point=24):
    ...
    np.random.seed(10)
    dat = np.loadtxt('acc0')
    #dat = (dat/2.**13*2**6).astype(int)    #this gives the error i saw
    dat = (dat/2.**13*2**13).astype(int)  
    dat = dat/2.**24
...
async def mov_test(dut, dat, win_len=16, din_width=32, din_int=1, thresh=0.05):
    ...
    for i in range(len(dat)):
        ...
        if(valid):
            ...
            out = two_comp_unpack(out, dout_width, dout_int)
            out_var.append(out)
    np.savetxt('rtl_var', out_var)
    np.savetxt('rtl_avg', out_avg)
    np.savetxt('gold_var', var_gold)
    np.savetxt('gold_avg', avg_gold)
