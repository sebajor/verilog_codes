...
@cocotb.test()
async def sobel_test(dut, file_in="../test_img.png", file_out='result.png',
        x_kernel='weight/y_sobel.txt', y_kernel="weight/x_sobel.txt", 
        din_width=8, din_pt=0, dout_width=13,dout_pt=1, thresh=0.5):
    ...
    x_kern = np.loadtxt(x_kernel)
    x_kern = x_kern.reshape([3,3])
    y_kern = np.loadtxt(y_kernel)
    y_kern = y_kern.reshape([3,3])
    gold_x = conv_operation(img, x_kern)
    gold_y = conv_operation(img, y_kern)

    gold = (gold_x+gold_y)/2
