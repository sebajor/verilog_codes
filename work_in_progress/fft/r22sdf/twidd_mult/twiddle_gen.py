...
def fix_twiddle_factors(stage_number, bit_width, bit_point):
    ...
    im_bin = [bin(int(x))[2:] for x in im_data]
...
def write_bin_non_trivial_twiddle(stage_number, bit_width, bit_point, folder='twiddles'):
    ...
    re_bin = (re_bin.reshape((-1,stage_number//2))[1:,1:]).flatten()
    im_bin = (im_bin.reshape((-1,stage_number//2))[1:,1:]).flatten()
    for re, im in zip(re_bin, im_bin):
        ##manually bit extend
        re_ext = "0"*(bit_width-len(re))+re
        im_ext = "0"*(bit_width-len(im))+im
        f.write(re_ext)
        f.write(im_ext)
        f.write('\n')
    f.close()
